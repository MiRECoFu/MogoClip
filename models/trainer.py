...
class Trainer:
    ...
    def forward(self, batch_data):
        ...
        cosine_sim = self.mogo_clip.mean_cosine_similarity(motion_code, captions)
        ...
        wandb.log({"Train/loss": loss, "Train/cosine_sim": cosine_sim})    
        return loss, cosine_sim
    
    
    def update(self, batch_data):
        loss, cosine_sim = self.forward(batch_data)
    ...
    def train(self, train_loader, val_loader, eval_loader):
        ...
        self.opt_mogo_clip = optim.AdamW(self.mogo_clip.parameters(), lr=self.lr, weight_decay=0)
        ...
        while epoch < self.max_epoch:
            ...
            for i, batch in enumerate(train_loader):
                it += 1
                loss, cosine_sim = self.update(batch_data=batch)
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(val_loader):
                ...
                val_cosine_sim.append(cosine_sim)

        print(f"Validation loss:{np.mean(val_loss):.3f}, cosine_sim:{np.mean(val_cosine_sim):.3f}")

        wandb.log({'Val/loss': np.mean(val_loss), 'Val/cosine_sim': np.mean(val_cosine_sim)})
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            print(f"Eval cosine_sim:{np.mean(eval_cosine):.3f}, R1:{np.mean(eval_r1):.3f}, R3:{np.mean(eval_r3):.3f}, R5:{np.mean(eval_r5):.3f}, R10:{np.mean(eval_r10):.3f}, MRR:{np.mean(eval_mrr):.3f}")

            wandb.log({'Eval/cosine_sim': np.mean(val_cosine_sim),'Eval/R1':np.mean(eval_r1),'Eval/R3':np.mean(eval_r3), 'Eval/R5':np.mean(eval_r5), 'Eval/R10': np.mean(eval_r10), 'Eval/MRR':np.mean(eval_mrr)})
